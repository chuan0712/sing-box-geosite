...
def parse_list_file(link, output_directory, custom_name=None):
    try:
        ...
        # 优先使用自定义别名，否则取 URL 文件名（去扩展名）
        base_name = custom_name if custom_name else os.path.basename(link).split('.')[0]
...
# 读取 links.txt，支持 "URL|别名" 格式
with open("../links.txt", 'r') as links_file:
    links_raw = links_file.read().splitlines()

# 过滤空行和注释行
links_raw = [l for l in links_raw if l.strip() and not l.strip().startswith("#")]
...
for l in links_raw:
    if '|' in l:
        url, alias = l.split('|', 1)
        links.append((url.strip(), alias.strip()))
    else:
        links.append((l.strip(), None))
